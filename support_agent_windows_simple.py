"""
AI Customer Support Agent - WINDOWS SIMPLIFIED VERSION
No pandas/numpy needed - runs with just anthropic library
...
class SupportAgent:
    """Simplified version for Windows - no pandas/numpy required"""
    ...
    def __init__(self, response_db_path='response_database.json', brand_name='Support'):
        ...
        try:
            ...
            print(f"Make sure response_database.json is in the same folder as this script")
            raise
    ...
    def classify_intent(self, message):
        """Classify using keyword matching only (no LLM)"""
        text = message.lower()
        
        keywords_map = {
            'Order Status': ['order', 'track', 'tracking', 'delivery', 'package', 'shipment', 'when', 'where', 'status', 'arrive'],
            'Returns & Refunds': ['return', 'refund', 'send back', 'exchange', 'damaged', 'wrong item', 'broken'],
            'Billing & Payment': ['charge', 'bill', 'payment', 'invoice', 'price', 'subscription', 'money', 'cost', 'double charged'],
            'Technical Issue': ['error', 'bug', 'crash', 'not working', 'app', 'website', 'login', 'slow', 'down'],
            'Account & Access': ['account', 'login', 'password', 'access', 'locked', 'verify', 'reset'],
            'Product Inquiry': ['feature', 'how to', 'help', 'question', 'what is', 'size', 'available', 'stock'],
            'Complaint & Escalation': ['angry', 'frustrated', 'terrible', 'awful', 'worst', 'manager', 'lawsuit', 'furious', 'hate'],
            'General Inquiry': []
        }
    ...
    def generate_reply(self, customer_message, intent):
        """Generate reply from templates (no LLM needed)"""
    ...
    def decide_escalation(self, customer_message, intent, reply):
        """Decide if message needs escalation"""
        ...
        for category, keywords in escalation_keywords.items():
            ...
            if matches > 0:
                ...
                if category == 'legal':
                    escalation_score += 20
                    escalation_reasons.append("Legal language detected")
    ...
    def process_message(self, customer_message):
        """Full pipeline: classify -> reply -> escalate"""
...
if __name__ == '__main__':
    # Test the agent
    agent = SupportAgent('response_database.json', brand_name='TechSupport')
    ...
    print("="*80)
    print("TESTING AI CUSTOMER SUPPORT AGENT (WINDOWS SIMPLIFIED)")
    print("="*80 + "\n")
